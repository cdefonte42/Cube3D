...
static bool	set_default_flags(t_game *game)
{
	...
	if (game->text[ewall].path == NULL)
		return (error("malloc failed", NULL));
	game->text[door].path = ft_strdup("img/door_64.xpm");
...
static bool init_sprite(t_game *game)
{
	...
	if (game->sprites == NULL)
		return (false);
	game->sort_sprite = ft_calloc(game->nb_sprites, sizeof(int));
	if (game->sort_sprite == NULL)
		return (false);
	k = 0;
	while (i < game->map.height)
	{
		...
		while (j < game->map.width)
		{
			...
			{
				game->sprites[k].pos.x = j + 0.5;
				game->sprites[k].pos.y = i + 0.5;
				++k;
			}
			++j;
		}
